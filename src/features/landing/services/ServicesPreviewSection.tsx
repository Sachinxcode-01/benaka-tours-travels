...
const ServiceCard: React.FC<ServiceCardProps> = ({
  ...
  return (
    <div
      ...
      className="group relative rounded-xl sm:rounded-2xl bg-[#0B0D12]/90 border border-white/10 p-3.5 sm:p-6 flex flex-col justify-between overflow-hidden shadow-xl transition-all duration-300 hover:border-[#D4AF37]/50 hover:shadow-2xl hover:shadow-amber-500/10"
    >
      ...
      <div className="space-y-2.5 sm:space-y-4 relative z-10">
        <div className="flex items-center justify-between">
          <div className="w-9 h-9 sm:w-12 sm:h-12 rounded-lg sm:rounded-xl bg-amber-500/10 border border-amber-500/30 flex items-center justify-center text-[#D4AF37] group-hover:scale-110 transition-transform duration-300 shadow-md">
            <IconComponent className="w-5 h-5 sm:w-6 sm:h-6" />
          </div>
          {service.badge && (
            <span className="text-[9px] sm:text-[10px] uppercase font-bold px-2 py-0.5 sm:px-2.5 rounded-full bg-amber-500/15 text-[#D4AF37] border border-amber-500/30">
              {service.badge}
            </span>
          )}
        </div>
        ...
        <div>
          <h3 className="text-sm sm:text-lg font-bold text-white group-hover:text-[#D4AF37] transition-colors leading-tight sm:leading-normal">
            {service.title}
          </h3>
          <p className="text-[11px] sm:text-xs text-slate-300 mt-1 sm:mt-1.5 leading-snug sm:leading-relaxed">
            {service.shortDescription}
          </p>
        </div>

        {/* Recommended Vehicle Badge */}
        <div className="p-2 sm:p-2.5 rounded-lg sm:rounded-xl bg-[#121620] border border-white/10 text-xs flex items-center gap-2">
          <ShieldCheck className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-emerald-400 shrink-0" />
          <div>
            <span className="text-[9px] sm:text-[10px] text-slate-400 block font-semibold uppercase">
              Recommended Fleet:
            </span>
      ...
      <div className="pt-3 sm:pt-6 mt-2.5 sm:mt-4 border-t border-white/10 grid grid-cols-2 gap-1.5 sm:gap-2 relative z-10">
        ...
        <a
          ...
          className="py-1.5 sm:py-2.5 px-2 sm:px-3 rounded-lg sm:rounded-xl bg-emerald-600/20 text-emerald-400 border border-emerald-500/30 font-semibold text-[11px] sm:text-xs flex items-center justify-center gap-1 sm:gap-1.5 hover:bg-emerald-600/30 active:scale-95 transition-all"
        >
...
export const ServicesPreviewSection: React.FC = () => {
  ...
  return (
    <section
      id="services"
      ref={containerRef}
      className="py-12 sm:py-20 bg-[#07080B] text-white relative"
    >
      <div className="max-w-7xl mx-auto px-4 space-y-8 sm:space-y-12">
        ...
        <div className="text-center space-y-2.5 sm:space-y-3 max-w-2xl mx-auto">
          <div className="inline-flex items-center gap-2 px-2.5 py-0.5 sm:px-3 sm:py-1 rounded-full bg-amber-500/10 text-[#D4AF37] border border-amber-500/30 text-[10px] sm:text-xs font-bold uppercase tracking-wider">
